...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
  ...
  protected void onAttachedToWindow() {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP && showShadow && shadowView != null) {
      ...
      if (params instanceof MarginLayoutParams) {
        ...
        int shadowHeight = getResources().getDimensionPixelSize(R.dimen.bb_fake_shadow_height);
  ...
  /**
   * Returns the settings_old specific for a shy BottomBar.
   *
   * @throws UnsupportedOperationException, if this BottomBar is not shy.
   */
  public ShySettings getShySettings() {
    if (!isShy()) {
      Log.e("BottomBar", "Tried to get shy settings_old for a BottomBar " +
          "that is not shy.");
    }
